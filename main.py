...
# eK - Added speedsum_link property to FileMetadata class
  speedsum_link = db.StringProperty()
...
class IndexHandler(webapp2.RequestHandler):
  ...
  def post(self):
    ...
    if self.request.get("word_count"):
     ...
     # eK - Added SpeedSumPipeline choice
    elif self.request.get("speedsum"):
	  pipeline = SpeedSumPipeline(filekey, blob_key)
...
# eK - Added split_into_lines(s) definition for FreewayData project
def split_into_lines(s):
  return s.split("\n")


# eK - Added split_into_columns(s) definition for FreewayData project
...
# eK - Added speed_sum_map(s) definition for FreewayData project
def speed_sum_map(data):
  """Speed summation map function"""
  (byte_offset, line_value) = data

  logging.debug("Got %s", line_value)
  cols = split_into_columns(line_value)
  if len(cols) > 3 and cols[3] != "speed":
    yield (cols[0], int(cols[3]))


# eK - Added speed_sum_reduce(s) definition for FreewayData project
def speed_sum_reduce(key, values):
  """Speed summation reduce function."""
  speedsum = 0
  for value in values:
	speedsum += int(value)
  speedcount = len(values)
  yield "%s: %d\n" % (key, speedsum)
...
# eK - Added SpeedPipeline class definition for FreewayData project
class SpeedSumPipeline(base_handler.PipelineBase):
  """A pipeline to run speed summation.
  ...
  def run(self, filekey, blobkey):
    output = yield mapreduce_pipeline.MapreducePipeline(
        ...
        shards=16)
    yield StoreOutput("SpeedSum", filekey, output)
...
class StoreOutput(base_handler.PipelineBase):
  ...
  def run(self, mr_type, encoded_key, output):
    ...
    # eK - Added mr_type choice for "SpeedSum"
    elif mr_type == "SpeedSum":
	  m.speedsum_link = output[0]
